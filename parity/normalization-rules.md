...
2. **Date/time format** — canonicalize to ISO 8601 before comparison. Legacy systems routinely emit `MMDDYY`, Julian dates, or locale-specific formats — none of that is a business difference.
3. **Currency representation** — strip symbols and thousands separators; compare as decimal values, not display strings.
...
Skipping straight from "run the transformation" to "diff the output" conflates two different failure modes: the AI got the *value* wrong, or the AI got the *format* right but different from whatever golden-dataset extraction happened to produce. Only the first one is a defect. Treating both the same way either buries real errors under a pile of formatting noise, or — worse — trains reviewers to stop reading discrepancy reports carefully, which is how a real defect eventually gets waved through.
