...
Every skill in this framework hands off its result the same way — as a JSON object matching `context/schemas/skill-message.schema.json`. This file explains the parts of that schema that need judgment, not just structure: what `confidence` means, why it's split into two `source` types, and how those types are allowed to interact.

## Why an envelope, not just a return value

Six skills, run in a fixed order, each depending on the one before. Without a shared shape, "what heuristic-validation hands to parity-evaluation" is whatever prose the last edit happened to describe, and nothing downstream can check it. With a shared envelope: every message is machine-parseable, every message is addressable by `run_id` for reassembling the full chain, and — the part that matters most — **every message states how much it trusts itself**, not just what it found.
...
**`self-reported`** — used by exactly one skill: `ai-semantic-validation`. When the skill reads an artifact's logic blind and describes what it thinks the code does, its confidence in that reading is a model's own self-assessment, not a formula over external evidence. That's inherently softer, and pretending otherwise would be the kind of false precision this whole framework exists to push back against.

## The propagation rule

**Self-reported confidence can lower a downstream score. It can never raise one.**

Concretely, in `confidence-scoring`: if `ai-semantic-validation` reports `agreement_level: aligned` with high self-reported confidence, that does not add points beyond what the Semantic Agreement component already awards for "aligned." But if it reports `contradicts` or flags `coincidentalMatchRisk`, that self-assessment is exactly what triggers the Low-band override — a downgrade signal is trusted, an upgrade signal from the same source is not. This mirrors the same asymmetry already built into the framework's core thesis: a positive numeric match is necessary but not sufficient, while a negative semantic finding is sufficient on its own to stop the pipeline.
...
Note the `confidence.value` here (0.55) is about how sure the skill is in its *own reading* — it is not softened by, and does not get overridden by, the severity of what it found. A skill can be highly confident in a bad finding, or moderately confident in a good one; the two numbers answer different questions and both get reported.
