...
3. **Severity determines what happens next.** `severity: block` means a failure halts the pipeline for this module; do not proceed to normalization or comparison. `severity: warn` means a failure is logged and the pipeline continues, but the warning must still appear in the final report regardless of what later stages find.
...
Confidence for this skill is always `calibrated`. Its basis is the heuristic's own track record, not just whether this one run passed: a brand-new heuristic passing once is weaker evidence than one that's cleared a hundred prior batches.
