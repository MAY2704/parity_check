...
If the Neo4j sync step fails while the TTL write succeeds, `status` is `partial`, `confidence.value` drops accordingly, and `gaps` states plainly that the graph in git and the graph in Neo4j are currently out of sync — never silently treat a partial write as complete.
