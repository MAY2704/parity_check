...
Normalization rules themselves live in `context/normalization-rules.md` — this file covers the comparison methodology and the metric formulas, which is the part that doesn't change per-module.
...
**Coverage rule:** a `BusinessRule` node with no linked `RuleEngineImpl` and no `ParityCheck` contributes its fields to **FN**, not to an excluded/unknown bucket. An unchecked field is treated as a missed defect for scoring purposes — this is what stops an unevaluated module from silently scoring as "no discrepancies found."
...
- **F1** is the balance point between precision and recall, used as the "Parity Match" component in confidence scoring rather than the raw match rate, specifically so a system can't game the score by only flagging the easy, obvious discrepancies.
