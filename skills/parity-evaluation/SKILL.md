...
7. **Report this message for the orchestrating agent to route onward**, to `knowledge-graph-builder` to write the `ParityCheck` evidence node, and to `ai-semantic-validation` for the logic-level cross-check. This skill does not invoke either directly; see `BEST_PRACTICES.md` §1.
